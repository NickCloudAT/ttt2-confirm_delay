...
if CLIENT then
	...
	hook.Add("TTTRenderEntityInfo", "TTT2_CONFDELAY_TARGETID", function(tData)
		...
		if not IsValid(ent) or ent:GetClass() ~= "prop_ragdoll" or not GetConVar("ttt_confirmdelay_enable"):GetBool() or
		GetRoundState() ~= ROUND_ACTIVE or (LocalPlayer():GetBaseRole() == ROLE_DETECTIVE and GetConVar("ttt_confirmdelay_detective_bypass"):GetBool()) then return end
